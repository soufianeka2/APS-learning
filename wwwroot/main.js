...
initViewer(document.getElementById('preview')).then(viewer => {
    ...
    viewer.addEventListener(Autodesk.Viewing.SELECTION_CHANGED_EVENT, function (event) {
        ...
        let domElem = document.getElementById('MySelectionValue');
    ...
    viewer.addEventListener(Autodesk.Viewing.NAVIGATION_MODE_CHANGED_EVENT, function (event) {
        let domElem = document.getElementById('MyToolValue');
        domElem.innerText = event.id;
    });
    if (registerCustomExtensions()) {
        loadCustomExtensions();
    }
    Scene.viewer = viewer;
    document.getElementById('add-custom-geometries').addEventListener('click', function(e) {
        loadCustomGeometries();
    });
...
function loadCustomGeometries() {
    let sphere = Sphere.createSphere();
    let sceneName = 'sphere-scene';
    customGeometries.push([sphere, sceneName]);
    if (Scene.createScene(sceneName)) {
        mountCustomGeometries(customGeometries);
    }
}
function unloadCustomGeometries() {
    dismountCustomGeometries(customGeometries, true);
}
